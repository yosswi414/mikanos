...
namespace {
    ...
    Error ScanDevice(uint8_t bus, uint8_t device) {
        ...
        if (IsSingleFunctionDevice(ReadHeaderType(bus, device, 0))) return MAKE_ERROR(Error::kSuccess);
    ...
    /**
     * @brief 指定された MSI capability 構造を読み取る
     *
     * @param dev MSI capability を読み込む PCI デバイス
     * @param cap_addr MSI capability レジスタの configuration 空間アドレス
    ...
    /**
     * @brief 指定された MSI capability 構造に書き込む
     *
     * @param dev MSI capability を書き込む PCI デバイス
     * @param cap_addr MSI capability レジスタの configuraiton 空間アドレス
     * @param msi_cap 書き込む値
     */
    ...
    Error ConfigureMSIRegister(const Device& dev, uint8_t cap_addr,
        ...
        msi_cap.header.bits.multi_msg_enable = min(msi_cap.header.bits.multi_msg_capable, num_vector_exponent);
...
namespace pci {
    ...
    uint32_t ReadBusNumbers(uint8_t bus, uint8_t device, uint8_t function) {
        WriteAddress(MakeAddress(bus, device, function, 0x18));  // Base Address Register 2
        return ReadData();
    }
    ...
    Error ScanAllBus() {
        ...
        for (uint8_t function = 1; function < 8; ++function) {
            // 0xffff: invalid vendor ID = not implemented function
            // see p.143 for more details
    ...
    Error ConfigureMSIFixedDestination(const Device& dev, uint8_t apic_id,
        ...
        msg_data.bits.level = trigger_mode == MSITriggerMode::kLevel;
        msg_data.bits.trigger_mode = trigger_mode;
